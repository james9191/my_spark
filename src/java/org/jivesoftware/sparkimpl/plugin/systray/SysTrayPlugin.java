	...
	if (SystemTray.isSupported()) {
		...
		trayIcon.addMouseListener(new MouseListener() {
		    ...
		    public void mouseReleased(MouseEvent event) {
		    	ChatManager.getInstance().showAllChatRoom();//显示所有未读消息
		    }
    ...
    public void flashWindow(Window window) {
    	if (pref.isSystemTrayNotificationEnabled()) {
			...
			if (window instanceof JFrame) {
				String title=getCounteredTitle(
						((JFrame) window).getTitle(), chatMessageHandler.getUnreadMessages());
				((JFrame) window).setTitle(title);
//				trayIcon.setToolTip("新消息:"+title);
				trayIcon.displayMessage("新消息", title, MessageType.INFO);
			}
			newMessage = true;
    	}
    }
