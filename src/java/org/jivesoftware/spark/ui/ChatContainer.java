...
//            chatFrame.setVisible(true);先隐藏
    ...
    public ChatRoom getChatRoom(String roomName) throws ChatRoomNotFoundException {
        ...
        //防止其它方法调用getChatRoom空指针
//    	room= ChatManager.getInstance().getWillChatRoom(roomName);
//    	if(null==room){
...
//		}
    ...
    protected void createFrameIfNeeded() {
        if (chatFrame != null) {
            return;
        }
	...
	if (SparkManager.getMainWindow().isActive() || pref.getWindowTakesFocus()) {
        ...
        chatFrame.setVisible(false);//先隐藏
